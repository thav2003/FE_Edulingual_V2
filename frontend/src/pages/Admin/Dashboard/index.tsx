...
const Dashboard: React.FC = () => {
  ...
  const fetchMonthFinances = () => {
    return dashboardApi.apiV1DashboardFinanceByMonthGet()
  }
  const fetchDashboard = () => {
    return dashboardApi.apiV1DashboardGet()
  }
  ...
  const [loadingMonthFinances, errorMonthFinances, responseMonthFinances] = useFetchData(fetchMonthFinances)
  const [loadingDashboard, errorDashboard, responseDashboard] = useFetchData(fetchDashboard)
  ...
  const dataMonthFinances = responseMonthFinances ? responseMonthFinances?.data?.data : {}
  ...
  const monthMap = {
    dataInJanuary: 'Tháng 1',
    dataInFebruary: 'Tháng 2',
    dataInMarch: 'Tháng 3',
    dataInApril: 'Tháng 4',
    dataInMay: 'Tháng 5',
    dataInJune: 'Tháng 6',
    dataInJuly: 'Tháng 7',
    dataInAugust: 'Tháng 8',
    dataInSeptember: 'Tháng 9',
    dataInOctober: 'Tháng 10',
    dataInNovember: 'Tháng 11',
    dataInDecember: 'Tháng 12'
  }
  const resultFinance = Object.entries(dataFinances).map(([key, value]) => ({
    day: dayMap[key],
    value: value
  }))
  const resultMonthFinance = Object.entries(dataMonthFinances).map(([key, value]) => ({
    day: monthMap[key],
    value: value
  }))
  ...
  console.log(resultMonthFinance)
            ...
            <Card bordered={false} className='criclebox h-full'>
              ...
              <Echart
                data={[
                  {
                    name: 'VNĐ',
                    data: resultMonthFinance.map((e) => e.value),
                    color: '#fff'
                  }
                ]}
                settings={{
